...
import {  FaUser, FaCalendarAlt, FaClipboardList, FaBell, FaBuilding, FaMapMarkerAlt, FaDollarSign, FaCreditCard, FaHistory, FaReceipt, FaDoorOpen } from 'react-icons/fa';
...
const MemberHome = () => {
  ...
  const { data: agreement = null } = useQuery({
    ...
    queryFn: async () => {
      const response = await axiosSecure.get(`/agreements?email=${user?.email}`);
      return response.data.find(agreement => agreement.status === 'checked') || null;
    },
    enabled: !!user?.email
  });
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              <Link to="/dashboard/paymentHistory" className="text-green-600 hover:text-green-800 text-sm font-medium">
                View History
              </Link>
            </div>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="text-center">
                <Link 
                  ...
                  onClick={() => {
                    // Navigate to home page and scroll to coupons section
                    setTimeout(() => {
                ...
                {availableCoupons.map((coupon, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="text-lg font-bold text-pink-700">{coupon.discountPercentage}% OFF</p>
                      </div>
                    </div>
                  </motion.div>
                ))}
          ...
          >
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                ...
                <span className="font-semibold text-gray-900">৳{agreement?.rent?.toLocaleString() || 'N/A'}</span>
              </div>
              
              <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                <span className="text-gray-700">Apartment</span>
                <span className="font-semibold text-blue-600">{agreement?.apartmentNo || 'N/A'}</span>
              </div>
